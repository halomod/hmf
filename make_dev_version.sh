...
# This function is make_dev_version.sh. It is used to convert a setuptools_scm
# version into a unique "dev" version identifier using the local hash info.
#
# Usage:
#   ./make_dev_version.sh
#
...
#
# This uses the "main" part of the version and the "local" git hash to generate
# a version number that is acceptable to the testpypi server. According to PEP
...
# version information.
#
# Examples showing the conversion from setuptools_scm version to pypi version:
#   1.0.2 -> 1.0.2
#   1.0.2.dev1+g0123456 -> 1.0.2.dev100010203040506
