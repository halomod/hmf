...
class MassDefinition(_framework.Component):
    ...
    def change_definition(self, m, mdef, profile=None, c=None):
        ...
        This requires using a profile, for which the `halomod` package must be used. If `halomod` is not installed,
        the default here is to use the NFW profile and the Duffy+08 concentration-mass relation,
        for which a hardcoded solution is inbuilt to this method.

        Parameters
        ----------
        m : float or array_like
            The halo mass to be changed, in :math:`M_\odot/h`. Must be broadcastable with `c`, if provided.
        ...
        profile : :class:`halomod.profiles.Profile` instance, optional
            An instantiated profile object from which to calculate the expected definition change. If not provided,
            a mocked NFW profile is used.

        c : float or array_like, optional
            The concentration(s) of the halos given. If not given, the concentrations will be automatically
            calculated using the profile object.
        ...
        """
        if c is not None and not np.isscalar(c) and not np.isscalar(m):
            if len(m) != len(c):
        ...
        if profile is None:
            ...
            c_new = np.array([_find_new_concentration(rho, mdef.halo_density, x_guess=cc) for rho, cc in zip(rhos, c)])
        else:
            if c is None:
            ...
            c_new = np.array(
                [_find_new_concentration(rho, mdef.halo_density, profile._h, cc) for rho, cc in zip(rhos, c)])
...
def _find_new_concentration(rho_s, halo_density, h=None, x_guess=5.0):
    ...
    if h is None:
        h = lambda x: (np.log(1.0 + x) - x / (1.0 + x))

    fnc = lambda x, rhos, density_threshold: rhos * h(x) * 3.0 / x ** 3 - density_threshold
    ...
    if x is None:
        raise OptimizationException('Could not determine x where the density threshold %.2f is satisfied.'%halo_density)
